...
The review generation function for the first order Markov chain takes in a starting word, number of sentences, and probablility for subsequent words. It works by randomly choosing the first word from the list of first words of real reviews, and generating the next word based on the transition probabilities. The probabilities are calculated from the chance that the current word in the state space will trasition to another word in the state space. The state space is every word or punctuation mark used in the collection of real wine reviews.
...
# Second-Order Markov chain reviews that were the best, and actaully made sense:
