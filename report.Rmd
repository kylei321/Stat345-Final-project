...
Due to some of our challenges with the second-order chain reviews, they don't sound as believable as the first-order reviews. This is not what we expected to be the case, as adding another word to generate transition probabilities should make the reviews more believable. However, that ended up not being the case for us. When limiting the words in the review, it sometimes cuts the review/sentence where it might not make sense for an actual sentence to end.
