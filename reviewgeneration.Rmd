...
# Start words are all available words besides punctuation
...
#Counts number of time each combination occurs (n)
count_2 <- review_second %>% count(`Part 1`, `Word 3`, sort = TRUE)

#Probablility of that combination ('pairs_2' is the final dataframe)
probability_2 <- (count_2$n/nrow(count_2))
pairs_2 <- cbind(count_2, probability_2)

#Finds the unique words of 'Part 1'
unique <- review_second %>% distinct(`Part 1`)
```

```{r}
#Creates large list of probabilities for each combination of second order markov chain
probabilities_2 <- apply(unique, 1, build_list)
names(probabilities_2) <- unlist(unique)
...
# Finds new probabilities (word pair count/first word count)
for(i in 1:nrow(pairs_2)) {
  first <- pairs_2$`Part 1`[i]
  second <- pairs_2$`Word 3`[i]
  probabilities_2[[first]][second, first] <- probabilities_2[[first]][second, first] / totals[[first]]
}
```
