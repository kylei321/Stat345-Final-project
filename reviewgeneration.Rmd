...
```{r}
# Builds the list of tables with all probabilities initialized to 0
build_list <- function(word) {
  init <- replicate(nrow(test), 0)
  probs <- data.frame(start = init)
  colnames(probs) <- word
  row.names(probs) <- unlist(test)
  return(probs)
}
...
# Fills in probabilities with those calculated above
